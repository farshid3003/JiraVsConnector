            ...
            FROM
                (
                    ...
                    FROM
                        vs_downloadsummary
                    WHERE
                        logtime <= now() --and logtime >= date_trunc('month', now() - interval '24 weeks')
                ) a
            JOIN
                (
                    SELECT
...
create or replace temporary view vs_featureusage3 
as 
select 
    to_char(uaf.month, 'YYYY-MM') as "Period",
    --to_char(100 * optinoutratio, '990D9%') as "Opt In Ratio",
        ...
        from 
            (
                ...
                from 
                    vs_downloadsummary
                where 
                --    logtime >= date_trunc('month', now() - interval '24 weeks')
                --and 
                    uid is not null
                group by 
                    month, uid
...
drop table if exists vs_feature_usage_existing;
select * into vs_feature_usage_existing from vs_featureusage3 